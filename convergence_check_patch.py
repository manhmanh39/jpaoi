# -*- coding: utf-8 -*-
"""
convergence_check_patch.py
================================================================================
Replacement for `experiment1_seed_convergence_check_all_scenarios` that finds
a defensible convergence point instead of eyeballing a rolling mean.

WHY THE OLD VERSION WAS NOT TRUSTWORTHY
----------------------------------------
The old function only ever looked at the CUMULATIVE point estimate
RE_delay_mean_pct(n) — the mean relative error using the first n seeds — and
then smoothed that single trajectory with a 3-point rolling mean. That has
three problems:
  1. No uncertainty quantification. There is no standard error attached to
     RE_delay_mean_pct(n), so "the rolling mean looks flat" is not a
     statistical statement, just a visual impression.
  2. The rolling window (3 points, 10 seeds apart = 30 seeds of smoothing)
     is arbitrary and provides no guarantee that the underlying stochastic
     process has actually stabilized rather than just slowed its drift.
  3. Cumulative-mean trajectories are autocorrelated by construction (each
     point reuses almost all of the previous point's seeds), so a flat
     region can hide a small but real remaining slope -- exactly what
     happened with S4 in the very data that motivated this rewrite: it
     "looked" flat around n=95, then kept climbing all the way to n~300.

WHAT THIS VERSION DOES INSTEAD
----------------------------------------
For every seed count n in `seed_counts`, we do NOT reuse the cumulative
mean. Instead we:
  (a) Collect the PER-SEED relative error value RE_delay(seed) for a batch
      of `seeds_per_block` fresh, non-overlapping seeds.
  (b) Treat each block's mean as one i.i.d. observation of the "true"
      RE_delay_mean at that scenario (blocking reduces the effect of
      per-seed correlation/outliers and gives us a distribution of block
      means we can use for a real standard error).
  (c) Track, for growing total seed count n, the running mean and its
      standard error SE(n) = std_of_block_means / sqrt(num_blocks), i.e.
      an SE computed from actual independent replicates, not assumed.
  (d) Declare convergence at the first n such that:
        - the 95% CI half-width (1.96 * SE(n)) is below an absolute
          tolerance `ci_tol` (in percentage points), AND
        - the running mean at n differs from the running mean at n/2
          (or the earliest available half-point) by less than the
          COMBINED CI half-width of the two estimates (a two-sample
          "no significant drift" check), AND
        - both conditions hold for `persist` consecutive checkpoints in a
          row (so a single lucky/unlucky block cannot trigger a false
          "converged" flag).
  (e) If no n in the tested range satisfies this, the scenario is reported
      as NOT CONVERGED, with the best available (n, mean, CI) reported
      honestly instead of silently accepting the last point.

This is still a heuristic (all MC convergence diagnostics are), but now it
is backed by an actual sampling distribution and a pre-registered numeric
threshold, instead of a visual "looks flat" judgment call.
"""
import numpy as np
import pandas as pd
